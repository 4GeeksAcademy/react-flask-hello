...
const Cart = () => {
  ...
  const handleCheckout = async () => {
    try {
      const token = localStorage.getItem("jwt") // El usuario debe estar logueado y tenga el token
      ...
      if (response.ok) {
        vaciarCarrito()
        navigate('/success') //Manda a SUCCESS.jsx
      } else {
  ...
  return (
    <div className="container my-4">
      <h1>Carrito</h1>
      <hr />
      <table className="table">  // Muestra lista de compra 
        <thead>
          <tr>
            <th>#</th>
            <th>Item</th>
